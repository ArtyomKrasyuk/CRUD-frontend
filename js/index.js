...
async function createFunc(){
    ...
    let url = 'http://127.0.0.1:8080/tables/firm';
...
async function readFunc(){
    let firmId = parseInt(document.querySelector('#id').value, 10);
    let response = await fetch(`http://127.0.0.1:8080/tables/firm/${firmId}`, {method: 'GET'});
    if(response.ok) {
...
async function updateFunc(){
    ...
    let firmId = parseInt(document.querySelector('#id-update').value, 10);
    let url = `http://127.0.0.1:8080/tables/firm/${firmId}`;
    let info = {'address': address, 'phone': phone, 'name': name};
    let response = await fetch(url, {
        method: 'PATCH',
        headers: {
...
async function deleteFunc(){
    let firmId = parseInt(document.querySelector('#id').value, 10);
    let response = await fetch(`http://127.0.0.1:8080/tables/firm/${firmId}`, {method: 'DELETE'});
